...
void ValueFilter::settings(const Setting& s)
{
  ...
  enabled_ = s.find(Setting("filter/enabled")).triggered();
  min_ = static_cast<uint32_t>(s.find(Setting("filter/min")).get_int());
...
Setting ValueFilter::settings(int32_t index) const
{
  ...
  SettingMeta en("filter/enabled", SettingType::boolean, "Enabled");
  Setting enabled(en);
  enabled.set_bool(enabled_);
  enabled.set_indices({index});
  ret.branches.add(enabled);

  SettingMeta mname("filter/value_id", SettingType::text, "Value ID");
  ...
  SettingMeta mmin("filter/min", SettingType::integer, "Minimum value");
  mmin.set_val("min", 0);
  ...
  SettingMeta mmax("filter/max", SettingType::integer, "Maximum value");
  mmax.set_val("min", 0);
