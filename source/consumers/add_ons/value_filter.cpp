...
{
  auto name = s.find(Setting("filter_value")).get_text();
  if (name != name_)
  {
    idx_ = -1;
    name_ = name;
  ...
  enabled_ = s.find(Setting("filter_enabled")).triggered();
  min_ = static_cast<uint32_t>(s.find(Setting("filter_min")).get_int());
  max_ = static_cast<uint32_t>(s.find(Setting("filter_max")).get_int());
}
...
Setting ValueFilter::settings(int32_t index) const
{
  ...
  SettingMeta en("filter_enabled", SettingType::boolean, "Enabled");
  Setting enabled(en);
  enabled.set_bool(enabled_);
  enabled.set_indices({index});
  ret.branches.add(enabled);

  SettingMeta mname("filter_value", SettingType::text, "Value ID");
  ...
  SettingMeta mmin("filter_min", SettingType::integer, "Minimum value");
  mmin.set_val("min", 0);
  ...
  SettingMeta mmax("filter_max", SettingType::integer, "Maximum value");
  mmax.set_val("min", 0);
