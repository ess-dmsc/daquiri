...
Histogram3D::Histogram3D() : Spectrum() {
...
void Histogram3D::_apply_attributes() {
  ...
  value_latch_z_.settings(
      metadata_.get_attribute(value_latch_z_.settings(2, "Z value")));
...
void Histogram3D::_recalc_axes() {
  ...
  auto calib2 = det2.get_calibration({value_latch_z_.value_id, det2.id()},
                                     {value_latch_z_.value_id});
