...
DenseMatrix2D::DenseMatrix2D() : Dataspace(2), spectrum_(100, 100) {}

bool DenseMatrix2D::empty() const { return (total_count_ == 0); }
...
void DenseMatrix2D::fill_list(EntryList &result, size_t min0, size_t max0,
                              size_t min1, size_t max1) const {
  for (size_t k = min0; k <= max0; ++k) {
    for (size_t l = min1; l <= max1; ++l) {
      result->push_back(
          {{static_cast<unsigned long>(k), static_cast<unsigned long>(l)},
           spectrum_.coeff(k, l)});
    }
  }
}
...
void DenseMatrix2D::data_save(const hdf5::node::Group &g) const {
  ...
  auto i_space =
      dataspace::Simple({static_cast<size_t>(spectrum_.nonZeros()), 2});
  dcpl.chunk({chunksize, 2});
  auto didx =
      g.create_dataset("indices", datatype::create<uint16_t>(), i_space, dcpl);
...
std::string DenseMatrix2D::data_debug(__attribute__((unused))
  ...
  ss << prepend << "Maximum=" << maximum << " rows=" << spectrum_.rows()
     << " cols=" << spectrum_.cols() << "\n";
  if (!maximum)
    return ss.str();
