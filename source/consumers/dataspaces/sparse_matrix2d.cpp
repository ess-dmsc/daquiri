...
SparseMatrix2D::SparseMatrix2D() : Dataspace(2) {}

bool SparseMatrix2D::empty() const { return (total_count_ == 0); }
...
void SparseMatrix2D::fill_list(EntryList &result, int64_t min0, int64_t max0,
                               int64_t min1, int64_t max1) const {
  for (int k = 0; k < spectrum_.outerSize(); ++k) {
    for (Eigen::SparseMatrix<double>::InnerIterator it(spectrum_, k); it;
         ++it) {
      ...
      if ((min0 > co0) || (co0 > max0) || (min1 > co1) || (co1 > max1))
        continue;
      result->push_back(
          {{static_cast<unsigned long>(co0), static_cast<unsigned long>(co1)},
           it.value()});
    }
  }
}
...
void SparseMatrix2D::data_save(const hdf5::node::Group &g) const {
  ...
  try {
    ...
    std::throw_with_nested(
        std::runtime_error("<SparseMatrix2D> Could not save"));
  }
}
...
void SparseMatrix2D::data_load(const hdf5::node::Group &g) {
  try {
    ...
    std::throw_with_nested(
        std::runtime_error("<SparseMatrix2D> Could not load"));
  }
}

std::string SparseMatrix2D::data_debug(__attribute__((unused))
