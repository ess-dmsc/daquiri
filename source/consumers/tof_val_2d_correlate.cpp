...
TOFVal2DCorrelate::TOFVal2DCorrelate() : Spectrum() {
  data_ = std::make_shared<SparseMap2D>();

  Setting base_options = metadata_.attributes();
  metadata_ = ConsumerMetadata(
      my_type(),
      "Time of flight 1D spectrum (with correlation across streams)");
  ...
  SettingMeta stream("chopper_stream_id", SettingType::text,
                     "Chopper stream ID");
...
void TOFVal2DCorrelate::_push_stats_post(const Spill &spill) {
  ...
  while (can_bin() && bin_events()) {
    //    DBG( "Binning " << events_buffer_.size() << " in " <<
    //    chopper_buffer_.size();
  }
