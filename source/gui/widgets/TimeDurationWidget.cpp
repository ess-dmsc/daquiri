...
TimeDurationWidget::TimeDurationWidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::TimeDurationWidget), us_enabled_(true) {
  ui->setupUi(this);
}

TimeDurationWidget::~TimeDurationWidget() { delete ui; }
...
void TimeDurationWidget::set_duration(hr_duration_t duration) {
  ...
  uint64_t total_us =
      static_cast<uint64_t>(round<std::chrono::microseconds>(duration).count());
...
void TimeDurationWidget::on_spin_us_valueChanged(int val) {
  if (val >= static_cast<int>(kFactorMetricOrder)) {
    ...
    ui->spin_ms->setValue(ui->spin_ms->value() +
                          new_value / kFactorMetricOrder);
    if (ui->spinDays->value() != ui->spinDays->maximum())
      ui->spin_us->setValue(new_value % kFactorMetricOrder);
    else
      ui->spin_us->setValue(0);
  } else if (val < 0) {
    if ((ui->spin_ms->value() > 0) || (ui->spinS->value() > 0) ||
        (ui->spinM->value() > 0) || (ui->spinH->value() > 0) ||
        (ui->spinDays->value() > 0)) {
...
void TimeDurationWidget::on_spin_ms_valueChanged(int val) {
  if (val >= static_cast<int>(kFactorMetricOrder)) {
    ...
    if ((ui->spinS->value() > 0) || (ui->spinM->value() > 0) ||
        (ui->spinH->value() > 0) || (ui->spinDays->value() > 0)) {
...
void TimeDurationWidget::on_spinS_valueChanged(int val) {
  if (val >= static_cast<int>(kFactorMinutesHours)) {
    ...
    if ((ui->spinM->value() > 0) || (ui->spinH->value() > 0) ||
        (ui->spinDays->value() > 0)) {
...
void TimeDurationWidget::on_spinH_editingFinished() { emit editingFinished(); }

void TimeDurationWidget::on_spinM_editingFinished() { emit editingFinished(); }

void TimeDurationWidget::on_spinS_editingFinished() { emit editingFinished(); }
