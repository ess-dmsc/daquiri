...
void SettingDelegate::text_len_limit(uint16_t tll) { text_len_limit_ = tll; }

uint16_t SettingDelegate::text_len_limit() const { return text_len_limit_; }
...
void SettingDelegate::paint_detector(QPainter *painter,
                                     const QStyleOptionViewItem &option,
...
void SettingDelegate::paint_indicator(QPainter *painter,
                                      const QStyleOptionViewItem &option,
                                      const Setting &val) const {
  auto ii = val.find(Setting(val.metadata().enum_name(val.get_number())));
  ...
  paintColor(painter, option.rect,
             (option.state & QStyle::State_Selected) ? inverseColor(color)
                                                     : color,
             get_string(val));
}

void SettingDelegate::paint_pattern(QPainter *painter,
                                    const QStyleOptionViewItem &option,
                                    const Setting &val) const {
...
void SettingDelegate::paint_command(QPainter *painter,
                                    const QStyleOptionViewItem &option,
                                    const Setting &val) const {
...
void SettingDelegate::paint_text(QPainter *painter,
                                 const QStyleOptionViewItem &option,
                                 const Setting &val) const {
  painter->save();
...
QSize SettingDelegate::sizeHint(const QStyleOptionViewItem &option,
  ...
  if (item.is(SettingType::command)) {
    ...
    PatternWidget pattern(item.pattern(), pattern_vis_size_,
                          pattern_chans_per_row_);
...

{
  ...
  if (set.is(SettingType::floating) || set.is(SettingType::precise)) {
    ...
    sb->setRange(set.metadata().min<int32_t>(), set.metadata().max<int32_t>());
    ...
    for (auto &q : set.metadata().enum_map())
      cb->addItem(QS(q.second), QVariant::fromValue(q.first));
    // there is a better way to do this, indeces might be bad
    if (set.metadata().enum_map().count(set.get_number())) {
      int cbIndex =
          cb->findText(QS(set.metadata().enum_name(set.get_number())));
      if (cbIndex >= 0)
        cb->setCurrentIndex(cbIndex);
    }
    return cb;
...
void SettingDelegate::setModelData(QWidget *editor, QAbstractItemModel *model,
  ...
  if (PatternWidget *pe = qobject_cast<PatternWidget *>(editor))
    ...
    model->setData(index, QVariant::fromValue(dte->get_duration()),
                   Qt::EditRole);
