...
void PatternWidget::set_pattern(DAQuiri::Pattern pattern, double size,
                                size_t wrap) {
...
int PatternWidget::flagAtPosition(int x, int y) {
  ...
  if (flag < 0 || flag > static_cast<int>(pattern_.size()) || (x < 40) ||
      (flag_x >= wrap_) || (flag_y >= rows_))
    return -1;
  ...
  for (int i = 0; i < rows_; ++i) {
    ...
    for (int j = 0; j < wrap_; ++j) {
      ...
      if (flag < static_cast<int>(pattern_.size())) {
        ...
        painter->setFont(
            QFont("Helvetica", std::floor(8.0 * (size_ / 25.0)), QFont::Bold));
