...
UnitConverter::make_ordered_map(std::string default_unit,
  ...
  for (std::size_t i = 0; i < prefix_values_indexed.size(); ++i) {
    bool min_criterion = (convert_prefix(min_SI_value, default_prefix,
                                         prefix_values_indexed[i]) >= 0.00001);
    bool max_criterion = (convert_prefix(max_SI_value, default_prefix,
                                         prefix_values_indexed[i]) <= 999999);
...
PreciseFloat UnitConverter::convert_units(PreciseFloat value, std::string from,
                                          std::string to) const {
  trim(from);
  trim(to);
  if (from == to)
...
std::string UnitConverter::get_prefix(std::string full_unit) const {
  //  std::u32string ucs4 =
  //  boost::locale::conv::utf_to_utf<char32_t>(full_unit); std::u32string
  //  prefix = ucs4;
...
PreciseFloat UnitConverter::convert_prefix(PreciseFloat value, std::string from,
                                           std::string to) const {
