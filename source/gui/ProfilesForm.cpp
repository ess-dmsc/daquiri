...
void ProfileDialog::clickedCancel() { accept(); }

ProfilesForm::ProfilesForm(QWidget *parent)
    : QDialog(parent), ui(new Ui::ProfilesForm) {
  ui->setupUi(this);
  ...
  ui->tableProfiles2->horizontalHeader()->setSectionResizeMode(
  ...
  connect(ui->tableProfiles2, SIGNAL(doubleClicked(QModelIndex)), this,
          SLOT(selection_double_clicked(QModelIndex)));

  update_profiles();
}

ProfilesForm::~ProfilesForm() { delete ui; }
...
void ProfilesForm::update_profiles() {
  ...
  for (auto i = 0; i < profiles_.size(); ++i) {
    QBrush background = (thisprofile == profiles_[i].id) ? QBrush(Qt::green)
                                                         : QBrush(Qt::white);

    add_to_table(ui->tableProfiles2, i, 0, profiles_[i].id, QVariant(),
                 background);

    add_to_table(ui->tableProfiles2, i, 1, profiles_[i].description, QVariant(),
                 background);
  }
...
void ProfilesForm::on_pushSelectRoot_clicked() {
  QString dirName = QFileDialog::getExistingDirectory(
      this, "Open Directory", Profiles::settings_dir(),
  ...
  if (!Profiles::is_valid_settings_dir(dirName)) {
    QMessageBox msgBox;
    msgBox.setText("Not a valid settings root. Must contain a valid 'profiles' "
                   "subdirectory.");
    msgBox.exec();
    return;
  }
