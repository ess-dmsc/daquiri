...
daquiri::daquiri(QWidget *parent, bool open_new_project, bool start_daq)
    : QMainWindow(parent), ui(new Ui::daquiri), log_stream_(), my_emitter_(),
      text_buffer_(log_stream_, my_emitter_),
      open_new_project_(open_new_project), start_daq_(start_daq), server(this) {
  ...
  ui->setupUi(this);
  connect(&my_emitter_, SIGNAL(writeLine(QString)), this,
          SLOT(add_log_text(QString)));

  server.start_listen(12345);
  connect(&server, SIGNAL(stopDAQ()), this, SLOT(stop_daq()));
  connect(&server, SIGNAL(startNewDAQ(QString)), this,
          SLOT(start_new_daq(QString)));
  connect(&server, SIGNAL(close_older(uint32_t)), this,
          SLOT(close_older(uint32_t)));
  ...
  connect(ui->tabs, SIGNAL(tabCloseRequested(int)), this,
          SLOT(close_tab_at(int)));
  ...
  ui->tabs->addTab(
      new QLabel("<center>Open new project by clicking \"+\"</center>"),
      QString());
  ui->tabs->setTabEnabled(0, false);
  // Add tab button to current tab. Button will be enabled, but tab -- not
  ui->tabs->tabBar()->setTabButton(0, QTabBar::RightSide, tb);

  connect(ui->tabs->tabBar(), SIGNAL(tabMoved(int, int)), this,
          SLOT(tabs_moved(int, int)));
  ...
  ui->tabs->setTabIcon(ui->tabs->count() - 1,
                       QIcon(":/icons/oxy/16/applications_systemg.png"));
  connect(main_tab_, SIGNAL(toggleIO(bool)), this, SLOT(toggleIO(bool)));
  connect(this,
          SIGNAL(toggle_push(bool, DAQuiri::ProducerStatus,
                             DAQuiri::StreamManifest)),
          main_tab_,
          SLOT(toggle_push(bool, DAQuiri::ProducerStatus,
                           DAQuiri::StreamManifest)));
...
void daquiri::closeEvent(QCloseEvent *event) {
  if (runner_thread_.running()) {
    int reply = QMessageBox::warning(
        this, "Ongoing data acquisition operations", "Terminate?",
...
void daquiri::add_log_text(QString line) { ui->logBox->append(line); }

void daquiri::update_settings(Setting sets, ProducerStatus status,
  ...
  if ((status & ProducerStatus::can_run) && open_new_project_ && start_daq_) {
...
void daquiri::saveSettings() {
  ...
  settings.setValue("current_profile",
                    Profiles::singleton().current_profile_name());
  settings.setValue("auto_boot", Profiles::singleton().auto_boot());
}

void daquiri::on_splitter_splitterMoved(int /*pos*/, int /*index*/) {
  ui->logBox->verticalScrollBar()->setValue(
      ui->logBox->verticalScrollBar()->maximum());
}
...
void daquiri::add_closable_tab(QWidget *widget) {
  ...
  cb->setToolTip("Close");
  cb->setFlat(true);
  connect(cb, SIGNAL(close_tab_widget(QWidget *)), this,
          SLOT(close_tab_widget(QWidget *)));
  ui->tabs->addTab(widget, widget->windowTitle());
  ui->tabs->tabBar()->setTabButton(ui->tabs->count() - 1, QTabBar::RightSide,
                                   cb);
}
...
void daquiri::tabs_moved(int, int) { reorder_tabs(); }
...
void daquiri::open_project(ProjectPtr proj, bool start, QString name) {
  ...
  connect(newSpectraForm, SIGNAL(requestClose(QWidget *)), this,
          SLOT(close_tab_widget(QWidget *)));
...
void daquiri::close_older(uint32_t mins) {
  ...
  for (int i = ui->tabs->count() - 1; i >= 0; --i) {
    ...
    if (ProjectForm *of = qobject_cast<ProjectForm *>(ui->tabs->widget(i))) {
      ...
      bool eval =
          ((date::floor<std::chrono::seconds>(dif).count() / 60) >= mins);
      if (eval) {
