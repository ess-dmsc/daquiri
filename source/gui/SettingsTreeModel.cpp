...
SettingsTreeItem::SettingsTreeItem(const Setting &data,
...
int SettingsTreeItem::childCount() const { return childItems.count(); }
...
QVariant SettingsTreeItem::display_data(int column) const {
  if (column == 0) {
    QString name = QS(itemData.metadata().get_string("name", ""));
    if (name.isEmpty())
      name = QS(itemData.id());
    if (itemData.is(SettingType::stem) && !itemData.get_text().empty())
    ...
    if (ret.size())
      ...
      return QVariant();
  } else if ((column == 2) && !itemData.is(SettingType::none) &&
...
QVariant SettingsTreeItem::edit_data(int column) const {
  if (column == 2)
    return QVariant::fromValue(itemData);
  else if ((column == 1) && (itemData.metadata().get_num("chans", 0) > 0)) {
    QList<QVariant> qlist;
    qlist.push_back(
        QVariant::fromValue(itemData.metadata().get_num("chans", 0)));
...
SettingsTreeItem *SettingsTreeItem::parent() { return parentItem; }
...
bool SettingsTreeItem::setData(int column, const QVariant &value) {
    ...
    foreach (QString idx, ilist) {
      ...
      if (ok && (i >= 0) &&
          (static_cast<int>(new_indices.size()) <
           itemData.metadata().get_num("chans", 0)))
  ...
  else if (itemData.is(SettingType::text) && (value.type() == QVariant::String))
    itemData.set_text(value.toString().toStdString());
  else if (itemData.is(SettingType::pattern) && (value.canConvert<Pattern>()))
...
SettingsTreeModel::~SettingsTreeModel() { delete rootItem; }
...
SettingsTreeItem *SettingsTreeModel::getItem(const QModelIndex &index) const {
  if (index.isValid()) {
    SettingsTreeItem *item =
        static_cast<SettingsTreeItem *>(index.internalPointer());
    if (item)
      return item;
  }
  return rootItem;
...
QModelIndex SettingsTreeModel::index(int row, int column,
...
/*bool SettingsTreeModel::insertRows(int position, int rows, const QModelIndex
&parent)
{
  SettingsTreeItem *parentItem = getItem(parent);
  bool success;
...
/*bool SettingsTreeModel::removeRows(int position, int rows, const QModelIndex
&parent)
{
  SettingsTreeItem *parentItem = getItem(parent);
  bool success = true;
...
bool SettingsTreeModel::setData(const QModelIndex &index, const QVariant &value,
                                int role) {
  if (role != Qt::EditRole)
    return false;
...
const Setting &SettingsTreeModel::get_tree() { return data_; }
