...
SettingsForm::SettingsForm(ThreadRunner &thread, QWidget *parent)
    : QWidget(parent), ui(new Ui::SettingsForm), runner_thread_(thread),
      tree_settings_model_(this), tree_delegate_(this) {
  ui->setupUi(this);
  ...
  connect(&runner_thread_,
          ...
          this,
          SLOT(update(DAQuiri::Setting, DAQuiri::ProducerStatus,
                      DAQuiri::StreamManifest)));
  ...
  ui->treeViewSettings->header()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  connect(&tree_delegate_, SIGNAL(begin_editing()), this,
          SLOT(begin_editing()));
  connect(&tree_delegate_, SIGNAL(ask_execute(DAQuiri::Setting, QModelIndex)),
          this, SLOT(ask_execute_tree(DAQuiri::Setting, QModelIndex)));
  ...
  connect(&tree_delegate_, SIGNAL(ask_dir(DAQuiri::Setting, QModelIndex)), this,
          SLOT(ask_dir_tree(DAQuiri::Setting, QModelIndex)));
  connect(&tree_delegate_, SIGNAL(ask_gradient(QString, QModelIndex)), this,
          SLOT(ask_gradient_tree(QString, QModelIndex)));
  connect(&tree_delegate_,
          SIGNAL(closeEditor(QWidget *, QAbstractItemDelegate::EndEditHint)),
          this,
          SLOT(stop_editing(QWidget *, QAbstractItemDelegate::EndEditHint)));

  connect(&tree_settings_model_, SIGNAL(tree_changed()), this,
          SLOT(push_settings()));
...
void SettingsForm::exit() { exiting_ = true; }

void SettingsForm::update(const Setting &tree, ProducerStatus status,
...
void SettingsForm::begin_editing() { editing_ = true; }
...
void SettingsForm::ask_binary_tree(Setting set, QModelIndex index) {
  ...
  editing_ = true;
  BinaryWidget *editor =
      new BinaryWidget(set, qobject_cast<QWidget *>(parent()));
  editor->setModal(true);
  editor->exec();

  if (!set.metadata().has_flag("readonly"))
    tree_settings_model_.setData(
        index, QVariant::fromValue(editor->get_setting().get_int()),
        Qt::EditRole);
  editing_ = false;
}
...
void SettingsForm::ask_file_tree(Setting set, QModelIndex index) {
  ...
  auto fd =
      new QFileDialog(qobject_cast<QWidget *>(parent()), QString("Chose File"),
                      QFileInfo(QS(set.get_text())).dir().absolutePath(),
                      QS(set.metadata().get_string("wildcards", "")));
  fd->setOption(QFileDialog::DontUseNativeDialog, true);
  fd->setFileMode(QFileDialog::ExistingFile);
  if (fd->exec() &&
      !fd->selectedFiles().isEmpty()) // && (validateFile(parent,
                                      // fd->selectedFiles().front(), false))
    tree_settings_model_.setData(
        index, QVariant::fromValue(fd->selectedFiles().front()), Qt::EditRole);
  editing_ = false;
}
...
void SettingsForm::ask_dir_tree(Setting set, QModelIndex index) {
  ...
  auto fd = new QFileDialog(qobject_cast<QWidget *>(parent()),
                            QString("Chose Directory"),
                            QFileInfo(QS(set.get_text())).dir().absolutePath());
  fd->setOption(QFileDialog::DontUseNativeDialog, true);
...
void SettingsForm::ask_gradient_tree(QString gname, QModelIndex index) {
  ...
  auto gs =
      new QPlot::GradientSelector(QPlot::Gradients::defaultGradients(), gname,
...
void SettingsForm::toggle_push(bool enable, ProducerStatus status,
                               StreamManifest manifest) {
  Q_UNUSED(manifest);
...
void SettingsForm::loadSettings() {
  ...
  ui->checkShowRO->setChecked(
      settings.value("settings_show_readonly", true).toBool());
  ui->pushExpandAll->setChecked(
      settings.value("settings_expand_all", false).toBool());
...
void SettingsForm::on_pushChangeProfile_clicked() {
  ...
  connect(profiles, SIGNAL(profileChosen(QString, bool)), this,
          SLOT(profile_chosen(QString, bool)));
  profiles->exec();
}

void SettingsForm::init_profile() {
  profile_chosen(Profiles::singleton().current_profile_name(),
                 Profiles::singleton().auto_boot());
}
...
void SettingsForm::on_pushRequestList_clicked() { emit requestList(); }
