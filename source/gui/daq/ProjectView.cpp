...
ProjectView::ProjectView(QWidget *parent)
  ...
  selector_->setSizePolicy(QSizePolicy::Preferred,
                           QSizePolicy::MinimumExpanding);
  selector_->setMaximumWidth(280);
  ui->scrollArea->setWidget(selector_);
  // ui->scrollArea->viewport()->setSizePolicy(QSizePolicy::Preferred,
  // QSizePolicy::MinimumExpanding);
  // ui->scrollArea->viewport()->setMinimumWidth(283);

  connect(selector_, SIGNAL(itemSelected(SelectorItem)), this,
          SLOT(selectorItemSelected(SelectorItem)));
  connect(selector_, SIGNAL(itemToggled(SelectorItem)), this,
          SLOT(selectorItemToggled(SelectorItem)));
  connect(selector_, SIGNAL(itemDoubleclicked(SelectorItem)), this,
          SLOT(selectorItemDoubleclicked(SelectorItem)));

  colors_menu_.addAction(QIcon(":/icons/show16.png"), "Show all", this,
                         SLOT(showAll()));
  colors_menu_.addAction(QIcon(":/icons/hide16.png"), "Hide all", this,
                         SLOT(hideAll()));
  colors_menu_.addAction(QIcon(":/icons/oxy/16/roll.png"), "Randomize colors",
                         this, SLOT(randAll()));
  ui->toolColors->setMenu(&colors_menu_);

  delete_menu_.addAction(QIcon(":/icons/oxy/16/editdelete.png"),
                         "Delete selected", this, SLOT(deleteSelected()));
  delete_menu_.addAction(QIcon(":/icons/show16.png"), "Delete shown", this,
                         SLOT(deleteShown()));
  delete_menu_.addAction(QIcon(":/icons/hide16.png"), "Delete hidden", this,
                         SLOT(deleteHidden()));
  ui->toolDelete->setMenu(&delete_menu_);

  tile_menu_.addAction(QIcon(), "Free-for-all", this, SLOT(tile_free()));
  tile_menu_.addAction(QIcon(":/icons/blank16.png"), "Tile grid", this,
                       SLOT(tile_grid()));
  tile_menu_.addAction(QIcon(":/icons/oxy/16/view_split_left_right.png"),
                       "Tile horizontal", this, SLOT(tile_horizontal()));
  tile_menu_.addAction(QIcon(":/icons/oxy/16/view_split_top_bottom.png"),
                       "Tile vertical", this, SLOT(tile_vertical()));
  enforce_tile_policy();
...
void ProjectView::enforce_all() {
  ...
  for (auto it = items.rbegin(); it != items.rend(); it++) {
    ...
    if (consumer->metadata().get_attribute("visible").get_bool() !=
        item.visible)
    ...
    if (consumers_.count(id) && !item.visible) {
      ...
      connect(consumer_widget, SIGNAL(destroyed(QObject *)), this,
              SLOT(consumerWidgetDestroyed(QObject *)));
  ...
  for (auto g : visible_groups) {
    if (!groups_.contains(g)) {
      ...
      consumer_widget->setAttribute(Qt::WA_DeleteOnClose);
      connect(consumer_widget, SIGNAL(destroyed(QObject *)), this,
              SLOT(groupWidgetDestroyed(QObject *)));
      auto w = ui->area->addSubWindow(consumer_widget);
      w->setContentsMargins(0, 0, 0, 0);
...
void ProjectView::selectorItemSelected(SelectorItem /*item*/) {
  ...
  if (!consumer) {
    ui->labelSpectrumInfo->setText("<html><head/><body><p>Left-click: see "
                                   "statistics here<br/>Right click: toggle "
                                   "visibility<br/>Double click: details / "
  ...
  QString infoText = "<nobr>" + itm.text + "(" + QS(consumer->type()) + ", " +
                     QString::number(bits) +
                     "bits)</nobr><br/>"
                     "<nobr>" +
                     detstr +
                     ...
                     "</nobr><br/>"
                     "<nobr>Rate (inst/total): " +
                     QString::number(rate_inst) + "cps / " +
                     QString::number(rate_total) +
                     "cps</nobr><br/>"
                     "<nobr>Live / real:  " +
                     QString::number(live) + "s / " + QString::number(real) +
                     "s</nobr><br/>"
                     "<nobr>Dead:  " +
...
void ProjectView::on_pushFullInfo_clicked() {
  ConsumerPtr consumer =
      project_->get_consumer(selector_->selected().data.toLongLong());
  if (!consumer)
    return;

  ConsumerDialog *newSpecDia =
      new ConsumerDialog(consumer, std::vector<Detector>(), detectors_,
  ...
  for (auto &q : project_->get_consumers())
    if (q) {
      ...
      if (appearance.metadata().has_flag("color")) {
        appearance.set_text(
            generateColor().name(QColor::HexArgb).toStdString());
...
void ProjectView::tile_grid(QMdiArea *area) { area->tileSubWindows(); }
    ...
    if (it.value() == o) {
      ...
      for (auto &c : project_->get_consumers()) {
        if (static_cast<size_t>(
                c->metadata().get_attribute("window_group").get_int()) == id)
          c->set_attribute(Setting::boolean("visible", false));
      }
      groups_.erase(it++);
    } else
