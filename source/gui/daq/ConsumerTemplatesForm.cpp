...
int ConsumerTemplatesTableModel::rowCount(
    const QModelIndex & /*parent*/) const {
  return consumers_.size();
}

int ConsumerTemplatesTableModel::columnCount(
    const QModelIndex & /*parent*/) const {
  return 7;
}

QVariant ConsumerTemplatesTableModel::headerData(int section,
...
QVariant ConsumerTemplatesTableModel::data(const QModelIndex &index,
                                           int role) const {
  int row = index.row();
  ...
  if (role == Qt::DisplayRole) {
    ...
    case 0:
      return QVariant::fromValue(
          consumers_.get(row)->metadata().get_attribute("name"));
    case 1:
      return QVariant::fromValue(
          consumers_.get(row)->metadata().get_attribute("visible"));
    case 2:
      return QS(consumers_.get(row)->metadata().type());
    case 3:
      return QVariant::fromValue(
          consumers_.get(row)->metadata().get_attribute("stream_id"));
    case 4:
      return QVariant::fromValue(
          consumers_.get(row)->metadata().get_attribute("appearance"));
    case 5:
      return QVariant::fromValue(
          consumers_.get(row)->metadata().get_attribute("preferred_scale"));
    }
  }
  return QVariant();
}
...
Qt::ItemFlags
ConsumerTemplatesTableModel::flags(const QModelIndex &index) const {
...
ConsumerTemplatesForm::ConsumerTemplatesForm(DAQuiri::ProjectPtr &project,
                                             ...
                                             QString profile_dir,
                                             QWidget *parent)
    : QDialog(parent), ui(new Ui::ConsumerTemplatesForm), project_(project),
      selection_model_(&table_model_), data_dir_(data_dir),
      profile_dir_(profile_dir), current_dets_(current_dets),
  ...
  ui->spectraSetupView->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  ui->spectraSetupView->verticalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  ...
  connect(ui->spectraSetupView, SIGNAL(doubleClicked(QModelIndex)), this,
          SLOT(selection_double_clicked(QModelIndex)));
...
ConsumerTemplatesForm::~ConsumerTemplatesForm() { delete ui; }
...
void ConsumerTemplatesForm::loadSettings() {
  ...
  ui->checkAutosaveDAQ->setChecked(
      settings.value("autosave_daq", true).toBool());
  ui->checkAskSaveProject->setChecked(
      settings.value("ask_save_project", true).toBool());
  auto idx = ui->comboRestart->findData(settings.value("on_restart", "ask"));
  ui->comboRestart->setCurrentIndex(idx);
  ui->spinDefaultLineThickness->setValue(
      settings.value("default_1d_thickness", 1).toInt());
  settings.endGroup();
}
...
void ConsumerTemplatesForm::saveSettings() {
  ...
  settings.setValue("default_1d_thickness",
                    ui->spinDefaultLineThickness->value());
  settings.endGroup();
}
...
void ConsumerTemplatesForm::toggle_push() {
  ...
  if (selection_model_.selectedRows().size() == 1) {
    ...
    if (ixl.front().row() > 0)
      ...
      ui->pushUp->setEnabled(false);
    if ((ixl.front().row() + 1) <
        static_cast<int>(project_->get_consumers().size()))
...
void ConsumerTemplatesForm::on_pushImport_clicked() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, "Import consumer prototypes", data_dir_, "DAQuiri project (*.daq)",
  ...
  try {
    ...
    for (const auto &c : project2->get_consumers()) {
      ...
      project_->add_consumer(
          ConsumerFactory::singleton().create_from_prototype(pr));
    }
...
void ConsumerTemplatesForm::on_pushNew_clicked() {
  ...
  ConsumerDialog *newDialog = new ConsumerDialog(
      nullptr, current_dets_, fakeDetDB, stream_manifest_, true, this);
  if (newDialog->exec()) {
...
void ConsumerTemplatesForm::on_pushEdit_clicked() {
  ...
  ConsumerDialog *newDialog =
      new ConsumerDialog(project_->get_consumer(i), current_dets_, fakeDetDB,
...
void ConsumerTemplatesForm::on_pushSetDefault_clicked() {
  ...
  for (auto cons : project_->get_consumers())
    project->add_consumer(ConsumerFactory::singleton().create_from_prototype(
        cons->metadata().prototype()));
...
void ConsumerTemplatesForm::on_pushUp_clicked() {
  ...
  selection_model_.setCurrentIndex(
      ixl.front().sibling(ixl.front().row() - 1, ixl.front().column()),
