...
ConsumerMulti1D::ConsumerMulti1D(QWidget *parent)
    : QWidget(parent), plot_(new QPlot::Multi1D()) {
...
void ConsumerMulti1D::update_data() {
  ...
  for (const auto &consumer : project_->get_consumers()) {
    ...
    if (static_cast<size_t>(md.get_attribute("window_group").get_int()) !=
        group_)
      continue;
...
void ConsumerMulti1D::refresh() { plot_->replot(QCustomPlot::rpQueuedRefresh); }
...
void ConsumerMulti1D::zoomedOut() { user_zoomed_ = false; }
