...
ProjectForm::ProjectForm(ThreadRunner &thread, ProjectPtr proj,
                         QString identity, QWidget *parent)
    : QWidget(parent), ui(new Ui::ProjectForm), runner_thread_(thread),
      interruptor_(false), project_(proj), project_identity_(identity) {
  ui->setupUi(this);

  menuLoad.addAction(QIcon(":/icons/oxy/16/document_open.png"),
                     "Open daquiri project", this, SLOT(projectOpen()));
  ui->toolOpen->setMenu(&menuLoad);

  menuSave.addAction(QIcon(":/icons/oxy/16/document_save.png"), "Save project",
                     this, SLOT(projectSave()));
  menuSave.addAction(QIcon(":/icons/oxy/16/document_save_as.png"),
                     "Save project as...", this, SLOT(projectSaveAs()));
  menuSave.addAction(QIcon(":/icons/oxy/16/document_save_all.png"),
                     "Save as json + csv", this, SLOT(projectSaveSplit()));
...
QString ProjectForm::profile_dir() const { return profile_dir_; }
...
void ProjectForm::closeEvent(QCloseEvent *event) {
  ...
  if (project_->changed() &&
    ...
    int reply =
        QMessageBox::warning(this, "Project contents changed", "Discard?",
                             QMessageBox::Yes | QMessageBox::Cancel);
    if (reply != QMessageBox::Yes) {
      event->ignore();
      return;
...
void ProjectForm::loadSettings() {
  ...
  data_directory_ =
      settings.value("save_directory", QDir::currentPath()).toString();
  settings.endGroup();
  ...
  ui->timeDuration->set_total_seconds(
      settings.value("run_secs", 60).toULongLong());
  ui->toggleIndefiniteRun->setChecked(
      settings.value("run_indefinite", true).toBool());
...
void ProjectForm::toggle_push(bool enable, ProducerStatus status,
  ...
  ui->pushStart->setEnabled(
      enable && !project_->empty() &&
      (profile_dir_ == Profiles::singleton().current_profile_dir()) &&
  ...
  ui->timeDuration->setEnabled(enable && (status & ProducerStatus::can_run) &&
...
void ProjectForm::projectOpen() {
  ...
  QString fileName = QFileDialog::getOpenFileName(
      this, "Load project", data_directory_, formats, nullptr,
...
void ProjectForm::projectSave() {
  if (!project_identity_.isEmpty()) {
    int reply = QMessageBox::warning(
        this, "Save?", "Save changes to existing project: " + project_identity_,
        QMessageBox::Yes | QMessageBox::Cancel);
    if (reply != QMessageBox::Yes)
      return;
...
void ProjectForm::projectSaveAs() {
  ...
  QString fileName =
      CustomSaveFileDialog(this, "Save project", data_directory_, formats);
...
void ProjectForm::projectSaveSplit() {
  ...
  QString fileName =
      CustomSaveFileDialog(this, "Save split", data_directory_, formats);
...
void ProjectForm::on_pushEditSpectra_clicked() {
  ConsumerTemplatesForm *newDialog =
      new ConsumerTemplatesForm(project_, current_dets_, stream_manifest_,
                                data_directory_, profile_dir_, this);
  newDialog->exec();
  ui->projectView->set_project(project_);
  update_plots();
}
...
void ProjectForm::newProject() { ui->projectView->set_project(project_); }
...
hr_time_t ProjectForm::opened() const { return opened_; }

bool ProjectForm::running() const { return !interruptor_.load(); }
