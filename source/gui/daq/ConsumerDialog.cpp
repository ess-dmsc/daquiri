...
ConsumerDialog::ConsumerDialog(ConsumerPtr consumer,
                               ...
                               bool allow_edit_type, QWidget *parent)
    : QDialog(parent), ui(new Ui::ConsumerDialog), consumer_(consumer)
      //,  det_selection_model_(&det_table_model_)
      ,
      attr_model_(this), detectors_(detDB),
      current_detectors_(current_detectors), stream_manifest_(stream_manifest),
      changed_(false) {
  ui->setupUi(this);
  loadSettings();
  ...
  connect(&attr_delegate_, SIGNAL(ask_gradient(QString, QModelIndex)), this,
          SLOT(ask_gradient(QString, QModelIndex)));
  connect(&attr_delegate_, SIGNAL(ask_file(DAQuiri::Setting, QModelIndex)),
          this, SLOT(ask_file(DAQuiri::Setting, QModelIndex)));
  connect(&attr_delegate_, SIGNAL(ask_dir(DAQuiri::Setting, QModelIndex)), this,
          SLOT(ask_dir(DAQuiri::Setting, QModelIndex)));
...
ConsumerDialog::~ConsumerDialog() { delete ui; }

DAQuiri::ConsumerPtr ConsumerDialog::product() { return consumer_; }
...
void ConsumerDialog::toggle_push() {
  ...
  //  QModelIndexList ixl =
  //  ui->tableDetectors->selectionModel()->selectedRows(); if (ixl.empty())
  //    return;
  //  int i = ixl.front().row();
...
void ConsumerDialog::on_pushLock_clicked() { open_close_locks(); }
...
void ConsumerDialog::on_buttonBox_accepted() {
  ...
  if (!changed_)
    ...
    ConsumerPtr newconsumer =
        ConsumerFactory::singleton().create_from_prototype(
            consumer_->metadata());
...
void ConsumerDialog::ask_file(Setting set, QModelIndex index) {
  ...
  if (fd->exec() &&
    ...
    attr_model_.setData(index, QVariant::fromValue(fd->selectedFiles().front()),
                        Qt::EditRole);
}
...
void ConsumerDialog::on_buttonBox_rejected() { reject(); }
...
void ConsumerDialog::initialize_gui_specific(DAQuiri::ConsumerMetadata &md) {
  ...
  if (col.metadata().has_flag("color")) {
    if (col.get_text().empty())
      ...
      col.set_text(
          QColor(QS(col.get_text())).name(QColor::HexArgb).toStdString());
...
void ConsumerDialog::on_pushDetEdit_clicked() {
  ...
  //  DialogDetector* newDet = new DialogDetector(spectrum_detectors_.get(i),
  //  false, this); connect(newDet, SIGNAL(newDetReady(Detector)), this,
  //  SLOT(changeDet(Detector))); newDet->exec();
}
