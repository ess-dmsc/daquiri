...
ListModeForm::ListModeForm(ThreadRunner &thread, QWidget *parent)
    : QWidget(parent), ui(new Ui::ListModeForm), runner_thread_(thread),
      interruptor_(false), my_run_(false), attr_model_(this) {
  ui->setupUi(this);
  ...
  ui->tableSpills->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  connect(ui->tableSpills->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)), this,
          SLOT(spill_selection_changed(QItemSelection, QItemSelection)));
  ...
  ui->tableEvents->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  connect(ui->tableEvents->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)), this,
          SLOT(event_selection_changed(QItemSelection, QItemSelection)));
  ...
  ui->tableTraces->horizontalHeader()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
  connect(ui->tableTraces->selectionModel(),
          SIGNAL(selectionChanged(QItemSelection, QItemSelection)), this,
          SLOT(trace_selection_changed(QItemSelection, QItemSelection)));
  ...
  ui->treeAttribs->header()->setSectionResizeMode(
      QHeaderView::ResizeToContents);
...
void ListModeForm::loadSettings() {
  ...
  ui->timeDuration->set_total_seconds(
      settings_.value("run_secs", 10).toULongLong());
  settings_.endGroup();
}
...
void ListModeForm::saveSettings() {
  ...
  settings_.beginGroup("ListDaq");
  settings_.setValue("run_secs",
                     QVariant::fromValue(ui->timeDuration->total_seconds()));
  settings_.endGroup();
}
...
ListModeForm::~ListModeForm() { delete ui; }

void ListModeForm::closeEvent(QCloseEvent *event) {
  if (my_run_ && runner_thread_.running()) {
    int reply =
        QMessageBox::warning(this, "Ongoing data acquisition", "Terminate?",
  ...
  if (!list_data_.empty()) {
    int reply = QMessageBox::warning(this, "Contents present", "Discard?",
                                     QMessageBox::Yes | QMessageBox::Cancel);
    if (reply != QMessageBox::Yes) {
      event->ignore();
      return;
...
void ListModeForm::displayHit(int idx) {
  ...
  ui->tableValues->setHorizontalHeaderItem(
      0, new QTableWidgetItem("Name", QTableWidgetItem::Type));
  ui->tableValues->setHorizontalHeaderItem(
      1, new QTableWidgetItem("Value", QTableWidgetItem::Type));
  ui->tableValues->setHorizontalHeaderItem(
      2, new QTableWidgetItem("Calibrated", QTableWidgetItem::Type));
  ...
  ui->tableTraces->setHorizontalHeaderItem(
      0, new QTableWidgetItem("Name", QTableWidgetItem::Type));
  ui->tableTraces->setHorizontalHeaderItem(
      1, new QTableWidgetItem("Rank", QTableWidgetItem::Type));
  ui->tableTraces->setHorizontalHeaderItem(
      2, new QTableWidgetItem("Dims", QTableWidgetItem::Type));
  ...
  for (size_t i = 0; i < event.trace_count(); ++i) {
    ...
    add_to_table(ui->tableTraces, i, 1,
                 QString::number(event_model_.traces[i].size()));
    QString dims;
...
void ListModeForm::list_completed(ListData newEvents) {
  if (my_run_) {
    ...
    ui->tableSpills->setHorizontalHeaderItem(
        0, new QTableWidgetItem("Stream", QTableWidgetItem::Type));
    ui->tableSpills->setHorizontalHeaderItem(
        1, new QTableWidgetItem("Type", QTableWidgetItem::Type));
    ui->tableSpills->setHorizontalHeaderItem(
        2, new QTableWidgetItem("Time", QTableWidgetItem::Type));
    size_t i = 0;
    for (auto &s : list_data_) {
      add_to_table(ui->tableSpills, i, 0, QS(s->stream_id));
      add_to_table(ui->tableSpills, i, 1, QS(Spill::to_str(s->type)));
      add_to_table(ui->tableSpills, i, 2, QS(to_iso_extended(s->time)));
      i++;
    }
...
void ListModeForm::spill_selection_changed(QItemSelection, QItemSelection) {
  ...
  ui->tableEvents->setHorizontalHeaderItem(
      0, new QTableWidgetItem("Time (native)", QTableWidgetItem::Type));
  ui->tableEvents->setHorizontalHeaderItem(
      1, new QTableWidgetItem("Time (ns)", QTableWidgetItem::Type));
  for (size_t i = 0; i < events_.size(); i++) {
    ...
    add_to_table(ui->tableEvents, i, 1,
                 QS(event_model_.timebase.debug()) + " = " +
                     QS(std::to_string(
                         event_model_.timebase.to_nanosec(event.timestamp()))));
    //                 QS(to_str_decimals(event_model_.timebase.to_nanosec(event.timestamp()),
    //                 3));
  }
...
void ListModeForm::trace_selection_changed(QItemSelection, QItemSelection) {
  ConsumerMetadata md =
      ConsumerFactory::singleton().create_prototype("Prebinned 1D");
  if (!ui->tableEvents->selectionModel()->selectedIndexes().empty() &&
      !ui->tableTraces->selectionModel()->selectedIndexes().empty()) {
    int spill_i =
        ui->tableSpills->selectionModel()->selectedIndexes().first().row();
    int event_i =
        ui->tableEvents->selectionModel()->selectedIndexes().first().row();
    int trace_i =
        ui->tableTraces->selectionModel()->selectedIndexes().first().row();

    SpillPtr sp = list_data_.at(spill_i);
    md.set_attribute(Setting::text("stream_id", sp->stream_id));
    md.set_attribute(
        Setting::text("trace_id", event_model_.trace_names.at(trace_i)));
    md.set_attribute(
        Setting::text("appearance", QColor(Qt::darkRed).name().toStdString()));
