...
void CommandServer::tcpReady() {
  if (server_socket) {
    auto text =
        QString(server_socket->read(server_socket->bytesAvailable())).trimmed();

    if (text == "STOP") {
      INFO("<CommandServer> received STOP");
      send_response(
          "<DAQuiri::CommandServer> OK: stopping all ongoing acquisitions\n");
      emit stopDAQ();
      ...
      send_response("<DAQuiri::CommandServer> OK: opening new project and "
      ...
      send_response(
          "<DAQuiri::CommandServer> ERROR: received unknown command '" + text +
          "'\n");
    }
  }
}
...
void CommandServer::acceptNew() {
  ...
  if (nc) {
    ...
    server_socket = nc;
    connect(server_socket, SIGNAL(error(QAbstractSocket::SocketError)), this,
...
void CommandServer::tcpError(QAbstractSocket::SocketError error) {
  ...
  if (server_socket) {
    if (server_socket->error() == QAbstractSocket::RemoteHostClosedError) {
      ...
      ERR("<CommandServer> TCP error: {}",
          server_socket->errorString().toStdString());
  }
}
