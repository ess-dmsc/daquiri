...
ConsumerPtr
ConsumerFactory::create_from_prototype(const ConsumerMetadata &tem) const {
...
ConsumerPtr ConsumerFactory::create_from_h5(hdf5::node::Group &group,
                                            bool withdata) const {
...
void ConsumerFactory::register_type(
    ConsumerMetadata tt, std::function<Consumer *(void)> constructor) {
  auto name = tt.type();
  if (name.empty())
