...
public:
  ...
  ConsumerPtr create_from_h5(hdf5::node::Group &group,
                             bool withdata = true) const;
...
template <class T> class ConsumerRegistrar {
public:
  ConsumerRegistrar() {
    ConsumerFactory::singleton().register_type(
        T().metadata(), [](void) -> Consumer * { return new T(); });
  }
};

#define DAQUIRI_REGISTER_CONSUMER(T)                                           \
  static DAQuiri::ConsumerRegistrar<T> UNIQUE_MANGLE(MangledDAQuiriConsumerReg);
