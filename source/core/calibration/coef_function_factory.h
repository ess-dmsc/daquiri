...
template <class T> class CoefFunctionRegistrar {
public:
  CoefFunctionRegistrar() {
    CoefFunctionFactory::singleton().register_type(
        [](void) -> CoefFunction * { return new T(); });
  }
};

#define DAQUIRI_REGISTER_COEF_FUNCTION(T)                                      \
  static DAQuiri::CoefFunctionRegistrar<T> UNIQUE_MANGLE(                      \
      MangledDAQuiriCoefFuncReg);
