...
Parameter::Parameter(double v) : value_(v) {}

Parameter::Parameter(double v1, double v2, double v3) { set(v1, v2, v3); }

Parameter::Parameter(double v1, double v2) { constrain(v1, v2); }
...
double Parameter::value() const { return value_; }

double Parameter::lower() const { return lower_; }

double Parameter::upper() const { return upper_; }

bool Parameter::fixed() const { return fixed_ || implicitly_fixed(); }

bool Parameter::implicitly_fixed() const {
  return (std::isfinite(value_) && (value_ == lower_) && (lower_ == upper_));
}
