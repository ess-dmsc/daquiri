...
CalibID::CalibID(std::string val, std::string det, std::string unit)
    : value(val), detector(det), units(unit) {}

bool CalibID::valid() const {
  return (!detector.empty() || !value.empty() || !units.empty());
}
...
bool CalibID::operator==(const CalibID &other) const {
  return (detector == other.detector) && (value == other.value) &&
         (units == other.units);
}
...
Calibration::Calibration(CalibID from, CalibID to) : from_(from), to_(to) {}
...
CalibID Calibration::from() const { return from_; }

CalibID Calibration::to() const { return to_; }

hr_time_t Calibration::created() const { return created_; }
...
void Calibration::function(CoefFunctionPtr f) { function_ = f; }

CoefFunctionPtr Calibration::function() const { return function_; }
...
std::vector<double>
Calibration::transform(const std::vector<double> &data) const {
...
void from_json(const nlohmann::json &j, Calibration &s) {
  ...
  if (j.count("function"))
    s.function_ =
        CoefFunctionFactory::singleton().create_from_json(j["function"]);
}

double shift_down(double v, uint16_t bits) { return v / pow(2, bits); }

double shift_up(double v, uint16_t bits) { return v * pow(2, bits); }
