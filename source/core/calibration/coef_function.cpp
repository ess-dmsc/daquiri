...
void CoefFunction::x_offset(const Parameter &o) { xoffset_ = o; }

Parameter CoefFunction::x_offset() const { return xoffset_; }

void CoefFunction::chi2(double c2) { chi2_ = c2; }

double CoefFunction::chi2() const { return chi2_; }

std::map<int, Parameter> CoefFunction::coeffs() const { return coeffs_; }
...
double CoefFunction::inverse(double y, double e) const {
  ...
  if (std::abs(x1 - x0) <= e)
    ...
    //    WARN("<" << this->type() << "> Maximum iteration reached in
    //    CoefFunction inverse evaluation";
    return nan("");
  }
}
