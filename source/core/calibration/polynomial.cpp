...
std::string Polynomial::debug() const {
  ...
  for (auto &c : coeffs_) {
    ...
    i++;
    vars +=
        "     p" + std::to_string(c.first) + "=" + c.second.to_string() + "\n";
  }
...
std::string Polynomial::to_UTF8(int precision, bool with_rsq) const {
  ...
  if (with_rsq)
    calib_eqn += std::string("   r") + UTF_superscript(2) + std::string("=") +
...
std::string Polynomial::to_markup(int precision, bool with_rsq) const {
  ...
  if (with_rsq)
    calib_eqn += "   r<sup>2</sup>" + std::string("=") +
                 to_str_precision(chi2_, precision);
...
double Polynomial::derivative(double x) const {
  ...
  for (auto &c : coeffs_) {
    if (c.first != 0) {
      new_poly.set_coeff(c.first - 1, {c.second.lower() * c.first,
