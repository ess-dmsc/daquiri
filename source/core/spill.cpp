...
Spill::Spill(std::string id, Spill::Type t) : stream_id(id), type(t) {
  if (t != Spill::Type::daq_status)
    state = Setting::stem("stats");
}

bool Spill::empty() { return (raw.empty() && events.empty() && !state); }
...
std::string Spill::debug(std::string prepend) const {
  ...
  ss << prepend << k_branch_mid_B << "event model: " << event_model.debug()
     << "\n";
  ...
  if (raw.size())
    ss << prepend << k_branch_mid_B
       << "raw_size=" << (raw.size() * sizeof(char)) << "\n";

  ss << prepend << k_branch_end_B << state.debug(prepend + "  ", false);
