...
ConsumerMetadata::ConsumerMetadata(std::string tp, std::string descr)
...
bool ConsumerMetadata::operator==(const ConsumerMetadata &other) const {
  if (type_ != other.type_)
    return false; // assume other type info same
...
Setting ConsumerMetadata::get_attribute(std::string setting,
                                        int32_t idx) const {
...
Setting ConsumerMetadata::attributes() const { return attributes_; }
...
void ConsumerMetadata::set_attributes(const std::list<Setting> &s,
                                      bool greedy) {
  for (auto &ss : s)
...
void to_json(json &j, const ConsumerMetadata &s) {
  try {
    ...
    std::throw_with_nested(std::runtime_error(
        "<ConsumerMetadata> Could not convert to json" + s.debug("")));
  }
}
...
void from_json(const json &j, ConsumerMetadata &s) {
  try {
    ...
    std::throw_with_nested(std::runtime_error(
        "<ConsumerMetadata> Could not convert from json" + j.dump(0)));
  }
}
...
bool ConsumerMetadata::chan_relevant(uint16_t chan) const {
  auto allpatterns =
      attributes_.find_all(Setting({"", SettingType::pattern}), Match::stype);
