...
public:
  ostream_sink(std::ostream *gui_stream) : outStream(gui_stream) {}
...
protected:
  ...
  void flush_() override { outStream->flush(); };
...
ostream_mt(const std::string &logger_name,
...
template <typename Factory = default_factory>
inline std::shared_ptr<logger>
ostream_st(const std::string &logger_name,
           const spdlog::level::level_enum &LoggingLevel,
           const std::string &host, const int &port) {
...
void initLogger(const spdlog::level::level_enum &LoggingLevel,
                const std::string &log_file_name, std::ostream *gui_stream) {
  closeLogger();
  ...
  if (gui_stream) {
    auto file_sink =
        std::make_shared<spdlog::sinks::ostream_sink_mt>(gui_stream);
    file_sink->set_pattern("[%Y-%m-%d %H:%M:%S.%f] [%l] [processID: %P]: %v");
    sinks.push_back(file_sink);
  }
