...
//#define LOGL(Severity, Format, ...) Log::Msg(Severity, fmt::format(Format,
//##__VA_ARGS__), {{"file", std::string(__FILE__)}, {"line",
//std::int64_t(__LINE__)}})
//
//#define CRITL(Format, ...) LOGL(Log::Severity::Critical, Format,
//##__VA_ARGS__) #define ERRL(Format, ...) LOGL(Log::Severity::Error, Format,
//##__VA_ARGS__) #define WARNL(Format, ...) LOGL(Log::Severity::Warning, Format,
//##__VA_ARGS__) #define INFOL(Format, ...) LOGL(Log::Severity::Info, Format,
//##__VA_ARGS__) #define DBGL(Format, ...) LOGL(Log::Severity::Debug, Format,
//##__VA_ARGS__)
