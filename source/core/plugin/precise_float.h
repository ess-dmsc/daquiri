...
inline double to_double(PreciseFloat pf) { return pf; }

inline PreciseFloat from_double(double d) { return d; }
...
using PreciseFloat = boost::multiprecision::number<
    boost::multiprecision::cpp_dec_float<FLOAT_PRECISION>>;
#endif
...
inline double to_double(PreciseFloat pf) { return pf.convert_to<double>(); }

inline PreciseFloat from_double(double d) { return PreciseFloat{d}; }

inline PreciseFloat from_string(const std::string str) {
  PreciseFloat ret{std::numeric_limits<double>::quiet_NaN()};
  try {
    ret = PreciseFloat(str);
  } catch (...) {
  }
  return ret;
}
...
inline double to_double(PreciseFloat pf) { return static_cast<double>(pf); }
...
inline void to_json(json &j, const PreciseFloat &s) { j = to_string(s); }
