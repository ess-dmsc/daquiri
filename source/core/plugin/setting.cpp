...
Setting::Setting(SettingMeta meta) { metadata_ = meta; }

Setting::operator bool() const { return ((*this) != Setting()); }

std::string Setting::id() const { return metadata_.id(); }

SettingMeta Setting::metadata() const { return metadata_; }

bool Setting::is(SettingType type) const { return (metadata().is(type)); }

void Setting::clear_indices() { indices_.clear(); }
...
bool Setting::has_index(int32_t i) const { return indices_.count(i); }

std::set<int32_t> Setting::indices() const { return indices_; }
...
void Setting::set_time(hr_time_t v) { value_time = v; }

hr_time_t Setting::time() const { return value_time; }
...
void Setting::set_duration(hr_duration_t v) { value_duration = v; }

hr_duration_t Setting::duration() const { return value_duration; }
...
void Setting::set_pattern(Pattern v) { value_pattern = v; }

Pattern Setting::pattern() const { return value_pattern; }
...
bool Setting::compare(const Setting &other, Match m) const {
  ...
  if ((m & Match::indices) && !compare_indices(other))
    return false;
  if ((m & Match::name) && (other.metadata_.get_string("name", "") !=
...
bool Setting::operator==(const Setting &other) const {
  ...
  if (indices_ != other.indices_)
    return false;
  if (value_dbl != other.value_dbl)
    return false;
  if (value_int != other.value_int)
    return false;
  if (value_text != other.value_text)
    return false;
  if (value_time != other.value_time)
    return false;
  if (value_duration != other.value_duration)
    return false;
  if (value_precise != other.value_precise)
    return false;
  if (value_pattern != other.value_pattern)
    return false;
...
bool Setting::find_dfs(Setting &result, const Setting &root, Match m) const {
...
void Setting::enrich(
    const std::map<std::string, SettingMeta> &setting_definitions,
    bool impose_limits) {
...
void Setting::set_text(std::string val) { value_text = val; }

std::string Setting::get_text() const { return value_text; }
...
bool Setting::numeric() const { return metadata_.is_numeric(); }

double Setting::get_number() const {
  if (is(SettingType::integer) || is(SettingType::binary) ||
      is(SettingType::menu) || is(SettingType::indicator))
...
void Setting::set_precise(PreciseFloat pf) { value_precise = pf; }

PreciseFloat Setting::precise() const { return value_precise; }

void Setting::set_int(integer_t v) { value_int = v; }

integer_t Setting::get_int() const { return value_int; }

void Setting::select(integer_t v) { value_int = v; }

integer_t Setting::selection() const { return value_int; }

bool Setting::get_bool() const { return (0 != value_int); }

void Setting::set_bool(bool b) { value_int = b; }

bool Setting::triggered() const { return (0 != value_int); }

void Setting::trigger() { value_int = 1; }

void Setting::reset() { value_int = 0; }
...
std::string Setting::val_to_string() const {
  ...
  if (is(SettingType::integer))
    ...
    if (value_int != 0)
      ...
      ss << "False";
  } else if (is(SettingType::text) || is(SettingType::stem))
    ss << value_text;
...
json Setting::val_to_json() const {
  ...
  else if (is(SettingType::integer) || is(SettingType::menu) ||
           is(SettingType::binary) || is(SettingType::indicator))
