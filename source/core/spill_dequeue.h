...
struct SmartSpillDeque {
  ...
  bool push(const SpillPtr &s, bool drop, size_t max_buffers) {
    if (drop && (s->type == Spill::Type::running) &&
...
public:
  inline SpillMultiqueue(bool drop, size_t max_buffers)
      : drop_(drop), max_buffers_(max_buffers) {}
  ...
  inline SpillPtr dequeue() {
    ...
    for (auto &s : streams_) {
      if (!s.second.empty() && ((earliest->earliest == hr_time_t()) ||
  ...
  inline size_t dropped_spills() { return dropped_spills_.load(); }

  inline size_t dropped_events() { return dropped_events_.load(); }
