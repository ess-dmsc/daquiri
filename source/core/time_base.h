...
public:
  ...
  inline PreciseFloat multiplier() const { return multiplier_; }

  inline PreciseFloat divider() const { return divider_; }
  ...
  inline bool operator==(const TimeBase &other) const {
    return ((divider_ == other.divider_) && (multiplier_ == other.multiplier_));
  }
...
public:
  TimeBasePlugin() {
    ...
    SettingMeta tm(r + "/multiplier", SettingType::integer,
                   "Timebase multiplier");
    tm.set_val("min", 1);
    tm.set_val("units", "ns");
    add_definition(tm);
  ...
  TimeBasePlugin(TimeBase tb) : TimeBasePlugin() { timebase_ = tb; }

  TimeBase timebase() const { return timebase_; }
