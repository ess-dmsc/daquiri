...
public:
  ...
  inline Event(const EventModel &model) : values_(model.values) {
  ...
  inline size_t trace_count() const { return traces_.size(); }

  inline uint32_t value(size_t idx) const { return values_[idx]; }
  ...
  // Setters
  inline void set_time(uint64_t t) { timestamp_ = t; }

  inline void set_value(size_t idx, uint32_t val) { values_[idx] = val; }
