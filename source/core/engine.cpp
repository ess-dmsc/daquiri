...
Engine::Engine() {
  ...
  SettingMeta e2{"MaxPackets", SettingType::integer,
                 "Maximum spills per stream"};
  e2.set_val("min", 1);
...
void Engine::initialize(const json &profile) {
  ...
  for (auto &q : tree.branches) {
    ...
    if (!device || name.empty()) {
      WARN("<Engine> Failed to load producer  type={}  name=\"{}\"", q.id(),
           name);
      continue;
    }
...
StreamManifest Engine::stream_manifest() {
  ...
  for (auto &q : producers_) {
    ...
    ret["engine"].stats.branches.add(
        SettingMeta("queue_size", SettingType::integer));
    ret["engine"].stats.branches.add(
        SettingMeta("dropped_spills", SettingType::integer));
    ret["engine"].stats.branches.add(
        SettingMeta("dropped_events", SettingType::integer));
  }
  return ret;
}
...
OscilData Engine::oscilloscope() {
  ...
  for (auto &q : producers_)
    if ((q.second != nullptr) &&
        (q.second->status() & ProducerStatus::can_oscil)) {
...
bool Engine::daq_start(SpillQueue out_queue) {
  ...
  for (auto &q : producers_)
    if ((q.second != nullptr) &&
        (q.second->status() & ProducerStatus::can_run)) {
...
void Engine::acquire(ProjectPtr project, Interruptor &interruptor,
  ...
  auto builder =
      std::thread(&Engine::builder_naive, this, &parsed_queue, project);
  ...
  while (daq_running()) {
    ...
    if (announcement_timer.timeout()) {
      if (timeout > 0)
        INFO("  RUNNING Elapsed: {}  ETA: {}  Dropped spills: {}  Dropped "
             "events: {}",
...
void Engine::builder_naive(SpillQueue data_queue, ProjectPtr project) {
  double time{0};
  uint64_t presort_events(0), presort_cycles(0);

  SpillPtr spill;
  spill = std::make_shared<Spill>("engine", Spill::Type::start);
  spill->state.branches.add_a(
      Setting::integer("queue_size", data_queue->size()));
  spill->state.branches.add_a(
      Setting::integer("dropped_spills", data_queue->dropped_spills()));
  spill->state.branches.add_a(
      Setting::integer("dropped_events", data_queue->dropped_events()));
  ...
  DBG("<Engine::builder_naive> Finished "
      ...
      "\n   secs/spill={}"
      "\n   events/sec={}",
      presort_cycles, presort_events, time, (time / double(presort_cycles)),
