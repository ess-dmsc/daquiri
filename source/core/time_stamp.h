...
public:
  ...
  inline TimeStamp(uint64_t native_time, TimeBase tb = TimeBase())
      : native_(native_time), timebase_(tb) {}

  inline void set_native(uint64_t t) { native_ = t; }

  inline TimeBase base() const { return timebase_; }

  inline uint64_t native() const { return native_; }

  inline double nanosecs() const { return timebase_.to_nanosec(native_); }
