...
Custom2DPlot::Custom2DPlot(Configuration & Config) : mConfig(Config) {
  ...
  mColorMap->data()->setSize(mConfig.mXDim, mConfig.mYDim); // we want the color map to have nx * ny data points
  mColorMap->data()->setRange(QCPRange(0, mConfig.mXDim - 1), QCPRange(0, mConfig.mYDim - 1)); //
  // add a color scale:
  mColorScale = new QCPColorScale(this);
  plotLayout()->addElement(0, 1, mColorScale); // add it to the right of the main axis rect
  mColorScale->setType(QCPAxis::atRight); // scale shall be vertical bar with tick/axis labels right (actually atRight is already the default)
  mColorMap->setColorScale(mColorScale); // associate the color map with the color scale
...
void Custom2DPlot::colorMap(int phase) {
  ...
  for (int xIndex = 0; xIndex < mConfig.mXDim; ++xIndex)
  {
    for (int yIndex = 0; yIndex < mConfig.mYDim; ++yIndex)
    {
      ...
      z = 2.0*xIndex*phase/10*(qCos(xIndex*0.1 + phase) - qSin(yIndex*0.1 + phase));
      mColorMap->data()->setCell(xIndex, yIndex, z);
    }
  }

  // rescale the data dimension (color) such that all data points lie in the span
  // visualized by the color gradient:
  mColorMap->rescaleDataRange();
}

void Custom2DPlot::addData(int phase) {
  colorMap(phase);
  replot();
  return;
}
