...
MockProducer::MockProducer() {
  ...
  SettingMeta si(r + "/SpillInterval", SettingType::floating,
                 "Interval between spills");
  ...
  SettingMeta dt(r + "/DeadTime", SettingType::floating,
                 "Dead time (% of real time)");
  dt.set_val("min", 0);
  dt.set_val("max", 100);
  dt.set_val("step", 0.01);
  add_definition(dt);

  SettingMeta lambda(r + "/Lambda", SettingType::floating,
                     "Decay constant (λ)");
  lambda.set_val("min", 0);
  lambda.set_val("step", 0.01);
  add_definition(lambda);

  SettingMeta lambda2(r + "/SpillLambda", SettingType::floating,
                      "Decay constant (λ) per spill");
...
StreamManifest MockProducer::stream_manifest() const {
  ...
  ret[stream_id_].stats.branches.add(
      SettingMeta("native_time", SettingType::floating));
  ret[stream_id_].stats.branches.add(
      SettingMeta("live_time", SettingType::floating));
  ret[stream_id_].stats.branches.add(
      SettingMeta("live_trigger", SettingType::floating));
  ret[stream_id_].stats.branches.add(
      SettingMeta("pulse_time", SettingType::floating));
...
bool MockProducer::daq_running() { return (running_.load()); }
...
void MockProducer::settings(const Setting &settings) {
  ...
  uint16_t val_count_ =
      std::max(int(set.find({r + "/ValueCount"}).get_number()), 1);
...
void MockProducer::boot() {
  if (!(status_ & ProducerStatus::can_boot)) {
    WARN("<MockProducer> Cannot boot MockProducer. Failed flag check (can_boot "
         "== 0)");
    return;
  }
  ...
  clock_ = 0;
  status_ =
      ProducerStatus::loaded | ProducerStatus::booted | ProducerStatus::can_run;
}
...
void MockProducer::worker_run(SpillQueue spill_queue) {
  ...
  while (!terminate_.load()) {
    ...
    double overshoot = ((event_definition_.timebase.to_sec(clock_) - seconds) /
                        seconds * 100.0);
    if (overshoot > 0)
...
SpillPtr MockProducer::get_spill(Spill::Type t, double seconds) {
  ...
  recent_pulse_time_ = clock_ =
      event_definition_.timebase.to_native(seconds * pow(10, 9));

  if (t == Spill::Type::running)
    fill_events(spill, seconds);
  spill->events.finalize();

  clock_ = event_definition_.timebase.to_native((seconds + spill_interval_) *
                                                pow(10, 9));
  fill_stats(*spill);
...
void MockProducer::fill_events(SpillPtr &spill, double seconds) {
  ...
  double event_interval =
      event_definition_.timebase.to_native(spill_interval_ * pow(10, 9)) /
      total_events;
...
bool MockProducer::eval_spill_lambda(uint32_t i, uint32_t total) {
  double diff =
      (spill_lambda_ * 0.01) +
      (1.0 - (spill_lambda_ * 0.01)) * (double(total - i) / double(total));
...
void MockProducer::fill_stats(Spill &spill) const {
  ...
  spill.state.branches.add(
      Setting::floating("pulse_time", double(recent_pulse_time_)));
}
