...
Setting ev44_events::settings() const
{
  std::string r{plugin_name()};
...
void ev44_events::settings(const Setting& settings)
{
  std::string r{plugin_name()};
...
StreamManifest ev44_events::stream_manifest() const
{
  StreamManifest ret;
...
uint64_t ev44_events::stop(SpillMultiqueue * spill_queue)
{
  if (started_)
  {
...
std::string ev44_events::schema_id() const
{
  return std::string(EventMessageIdentifier());
}

std::string ev44_events::get_source_name(void* msg) const
{
  auto em = GetEventMessage(msg);
  auto NamePtr = em->source_name();
  if (NamePtr == nullptr)
  {
    ERR("<ev44_events> message has no source_name");
    return "";
  }
  return NamePtr->str();
}
...
uint64_t ev44_events::process_payload(SpillMultiqueue * spill_queue, void* msg)
{
  Timer timer(true);
...
//    DBG( "Time " << stats.time_start << " - " << stats.time_end;
  ...
  if (spoof_clock_ == Monotonous)
    run_spill->state.branches.add(Setting::precise("reference_time", time_high));
  else if (spoof_clock_ == Earliest)
    run_spill->state.branches.add(Setting::precise("reference_time", stats.time_start));
  else
    run_spill->state.branches.add(Setting::precise("reference_time", em->reference_time()));
...
size_t ev44_events::events_in_buffer(const EventMessage* em)
{
  auto t_len = em->time_of_flight()->Length();
  auto p_len = em->pixel_id()->Length();
...
bool ev44_events::in_order(const EventMessage* em)
{
...
std::string ev44_events::debug(const EventMessage* em)
{
  std::stringstream ss;
