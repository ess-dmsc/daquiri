...
ev42_events::ev42_events() {
  ...
  SettingMeta hb(r + "/Heartbeat", SettingType::boolean,
                 "Send empty heartbeat buffers");
  add_definition(hb);
  ...
  SettingMeta oor(r + "/MessageOrdering", SettingType::menu,
                  "If message_id out of order");
...
void ev42_events::settings(const Setting &settings) {
  ...
  ordering_ =
      static_cast<CheckOrdering>(set.find({r + "/MessageOrdering"}).get_int());
...
StreamManifest ev42_events::stream_manifest() const {
  ...
  ret[stream_id_].stats.branches.add(
      SettingMeta("pulse_time", SettingType::precise));
  return ret;
}
...
uint64_t ev42_events::process_payload(SpillQueue spill_queue, void *msg) {
  ...
  if ((ordering_ != Ignore) && !in_order(em)) {
    WARN("Buffer out of order ({}<={}) {}", em->message_id(), latest_buf_id_,
         debug(em));
    if (ordering_ == Reject) {
      stats.time_spent += timer.s();
      return 0;
  ...
  SpillPtr run_spill =
      std::make_shared<Spill>(stream_id_, Spill::Type::running);
  ...
  run_spill->state.branches.add(
      Setting::precise("native_time", stats.time_end));
  run_spill->state.branches.add(
      Setting::precise("dropped_buffers", stats.dropped_buffers));
  ...
  if (spoof_clock_ == Monotonous)
    ...
    run_spill->state.branches.add(
        Setting::precise("pulse_time", stats.time_start));
  else
    run_spill->state.branches.add(
        Setting::precise("pulse_time", em->pulse_time()));
  ...
  if (!started_) {
    ...
    //    start_spill->state.branches.add(Setting::text("source_name",
    //    source_name));
