...
bool ESSStream::daq_start(SpillQueue out_queue) {
  ...
  for (auto &s : streams_) {
    ...
    s.runner = std::thread(&ESSStream::Stream::worker_run, &s, out_queue,
                           kafka_config_.kafka_timeout_, &terminate_);
    total++;
  }
...
bool ESSStream::daq_running() { return (running_.load()); }
...
void ESSStream::boot() {
  if (!(status_ & ProducerStatus::can_boot)) {
    WARN(
        "<ESSStream> Cannot boot ESSStream. Failed flag check (can_boot == 0)");
    return;
  }
  ...
  for (auto &s : streams_) {
    if (!s.parser) {
      WARN("<ESSStream> Cannot boot ESSStream. No flat buffer parser "
           "specified.");
      continue;
    }
    ...
    if (!s.consumer) {
      ERR("<ESSStream:{}> Failed to start consumer.",
          s.config.kafka_topic_name_);
      continue;
    }

    INFO("<ESSStream:{}> booted with consumer {}", s.config.kafka_topic_name_,
         s.consumer->low_level->name());
    total_valid++;
  }
...
void ESSStream::Stream::worker_run(SpillQueue spill_queue,
  ...
  while (!terminate->load()) {
    ...
    if (get_fb_id(message) != parser->schema_id()) {
      //      auto ch = reinterpret_cast<char const
      //      *const>(message->low_level->payload()); std::string c(ch,
      //      message->low_level->len()); WARN("<ESSStream:{}> fb schema id
      //      mismatch {}!={}\ncontents:{}",
      //          config.kafka_topic_name_, get_fb_id(message),
      //          parser->schema_id());
      continue;
    }

    spills +=
        parser->process_payload(spill_queue, message->low_level->payload());
  ...
  DBG("<ESSStream:{}> Finished run, spills={}", config.kafka_topic_name_,
      spills);

  DBG("<ESSStream:{}>   time={}  secs/spill={}  skipped buffers={}",
      config.kafka_topic_name_, parser->stats.time_spent,
      parser->stats.time_spent / double(spills), parser->stats.dropped_buffers);
}
...
std::string ESSStream::get_fb_id(Kafka::MessagePtr message) {
  if (message->low_level->len() < 8) {
    ERR("Could not extract id. Flatbuffer was only {} bytes. Expected ≥ 8 "
        "bytes.",
        message->low_level->len());
    return {};
  }
...
uint64_t ESSStream::Stream::ff_stream(Kafka::MessagePtr message,
  ...
  if ((offsets.hi - message->low_level->offset()) > kafka_max_backlog) {
    DBG("<ESSStream> topic:{} partition:{} Backlog exceeded with offset={}  "
        "hi={}",
        message->low_level->topic_name(), message->low_level->partition(),
