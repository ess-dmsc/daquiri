...
class Message {
public:
  Message(RdKafka::Message *ptr) : low_level(ptr) {}
...
class Consumer {
public:
  Consumer(RdKafka::KafkaConsumer *ptr) : low_level(ptr) {}
  ...
  void seek(const std::string &topic, int32_t partition, int64_t offset,
            int timeout_ms);
...
public:
  ...
  get_partitions(std::shared_ptr<RdKafka::KafkaConsumer>, std::string topic);
  std::unique_ptr<RdKafka::Metadata>
      get_kafka_metadata(std::shared_ptr<RdKafka::KafkaConsumer>) const;
