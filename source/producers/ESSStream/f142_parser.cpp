...
ChopperTDC::ChopperTDC() : fb_parser() {
  ...
  SettingMeta chopperTDCStreamid(
      r + "/EventsStream", SettingType::text,
      "DAQuiri stream ID for Chopper TDC time stamps");
...
uint64_t ChopperTDC::process_payload(SpillQueue spill_queue, void *msg) {
  ...
  ret->state.branches.add(
      Setting::precise("native_time", ChopperTDCTimeStamp->timestamp()));
  ret->state.branches.add(
      Setting::precise("dropped_buffers", stats.dropped_buffers));
  ...
  if (!started_) {
    ...
    start_spill->state.branches.add(
